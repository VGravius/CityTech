...
### ⏱️ 8. Таймаут запроса (клиент отменил запрос) - ставим 3 секудны чтобы увидеть разницу, только для этого эксперимента
...
**Пояснение:**
	...
	Таймаут не сработал в данном тесте, потому что сервер обрабатывает запросы мгновенно (время ответа составляет миллисекунды). В текущей реализации отсутствуют тяжёлые операции (запросы к базам данных, сложные вычисления, ожидание внешних сервисов), которые могли бы задержать ответ.
	Однако код содержит обработку таймаута через проверку context->IsCancelled(). Если в будущем появятся долгие операции, сервер корректно вернёт статус DEADLINE_EXCEEDED при отмене запроса клиентом.
